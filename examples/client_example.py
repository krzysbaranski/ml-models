...
def recognize_gestures(image_path, api_url="http://localhost:8000/recognize_gesture",
                       return_image=True, image_format="jpeg"):
    """
    Send an image to the API for gesture recognition
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Client for Object Detection, Face Detection, and Gesture Recognition API')
    parser.add_argument('image_path', help='Path to the image file')
    parser.add_argument('--mode', choices=['objects', 'faces', 'gestures'], default='objects',
                        help='Detection mode: objects, faces, or gestures (default: objects)')
    ...
    if args.mode == 'faces':
        ...
        result = recognize_gestures(args.image_path)
    else:
        result = detect_objects(args.image_path)
    
    if result:
        # Handle different result formats
        if args.mode == 'gestures':
            print(f"\nDetected {result['count']} hand(s):")
            
            for i, recognition in enumerate(result['recognitions'], 1):
                print(f"\n  Hand {i}:")
                print(f"    Gesture: {recognition['gesture']}")
                print(f"    Confidence: {recognition['score']:.2%}")
                if 'handedness' in recognition:
                    print(f"    Hand: {recognition['handedness']} ({recognition['handedness_score']:.2%})")
                if 'landmarks' in recognition:
                    print(f"    Landmarks: {len(recognition['landmarks'])} points detected")
        else:
